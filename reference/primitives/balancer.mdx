---
title: "Balancer"
description: "Custom load balancers for routing traffic to services"
---

A Balancer accepts incoming traffic and balances it between the [Processes](/reference/primitives/process) of a [Service](/reference/primitives/service).
...
<ParamField path="annotations" type="list">
  A list of annotation keys and values to populate the metadata for the deployed balancer
</ParamField>

<ParamField path="ports" type="map" required>
  A map of ports in the format `listen:forward` where `listen` is the port that the balancer will listen on and `forward` is the port that the traffic will be forwarded to on the Service
</ParamField>

<ParamField path="service" type="string" required>
  The name of the service that will receive the traffic
</ParamField>

<ParamField path="whitelist" type="list">
  A list of CIDR ranges from which to limit inbound traffic to this balancer
</ParamField>
...
To configure TCP ports on a balancer:
