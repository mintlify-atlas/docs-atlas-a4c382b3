---
title: "Timer"
description: "Schedule processes to run at specific intervals using cron syntax"
---

A Timer spawns a [Process](/reference/primitives/process) on a schedule that is defined using [cron syntax](https://crontab.guru).
...
<ParamField path="annotations" type="list">
  A list of annotation keys and values to populate the metadata for the deployed pods and their serviceaccounts. Supported version >= 3.13.5
</ParamField>

<ParamField path="command" type="string" required>
  The command to execute once the Process starts
</ParamField>

<ParamField path="schedule" type="string" required>
  A cron formatted schedule for spawning the Process. All times are UTC
</ParamField>

<ParamField path="service" type="string" required>
  The name of the Service that will be used to spawn the Process
</ParamField>

<ParamField path="concurrency" type="string" default="Allow">
  Specifies how to treat concurrent executions of a job. Options: `Allow`, `Forbid`, `Replace`. Check the [Kubernetes documentation](https://kubernetes.io/docs/tasks/job/automated-tasks-with-cron-jobs/#concurrency-policy) for more info
</ParamField>

<ParamField path="parallelCount" type="number" default="1">
  The number of parallel replicas to run for each timer execution. Each replica receives a unique `TIMER_INDEX` environment variable (0-based). Supported version >= 3.22.4
</ParamField>
...
In this example, the `jobs` Service is scaled to zero and not running any durable Processes. The `cleanup` Timer will spawn a Process of the `jobs` Service to run `bin/cleanup` once every two minutes.
